...
async function main() {
    ...
    // Get the actual hospital IDs (in case they were auto-generated)
    const chestnutHill = await prisma.hospital.findUnique({ where: { name: 'Chestnut Hill' } });
    const patriotPlace = await prisma.hospital.findUnique({ where: { name: 'Patriot Place' } });
    
    if (!chestnutHill || !patriotPlace) {
        throw new Error('Failed to find hospitals after seeding');
    }
    
    const chestnutHillId = chestnutHill.hospitalId;
    const patriotPlaceId = patriotPlace.hospitalId;
    
    console.log(`Chestnut Hill ID: ${chestnutHillId}, Patriot Place ID: ${patriotPlaceId}`);
