...
export default function SecurityServiceRequest() {
    ...
    const onSubmit = (e: FormEvent<HTMLFormElement>) => {
        ...
        apiClient
            .post(API_ROUTES.SERVICEREQS + "/security", payload)
            .then((response) => {
                console.log('Security request created:', response.data);
                setSubmitted(true);
            })
            .catch((err) => {
            ...
            {!submitted ?
                ...
                <div className="flex flex-col gap-4 items-center py-6 px-4">
                    <div className="bg-[#0077b6]/90 text-white rounded-3xl px-5 py-3 w-full shadow-lg border border-[#03045e]/60">
                        <h2 className="text-2xl sm:text-3xl font-bold text-left">Request Security Presence</h2>
                    </div>
                    <form onSubmit={onSubmit} className="w-full space-y-3">
                        ...
                        <div>
                            ...
                            <Input
                                ...
                                type="text"
                                id="employeeName"
                                className = "w-80 h-10 rounded-full border-2 border-[#0077b6] bg-white px-4 py-2 transition-all duration-300 focus:border-[#00b4d8] focus:ring-2 focus:ring-[#90e0ef]/50 focus:outline-none shadow-sm hover:shadow-md"                                onChange={(e) =>
                                    setForm({
                                        ...form,
